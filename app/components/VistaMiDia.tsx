...
export default function VistaMiDia({
  ...
  // Extraemos exclusivamente del expediente real del paciente (datosPaciente / PDF)
  const estructuraDias = datosPaciente?.dias || datosPaciente?.planSemanal || datosPaciente?.menu || {};
  const comidasDelDia = estructuraDias[diaSeleccionado];

  const menuOficial = comidasDelDia 
    ? (Array.isArray(comidasDelDia) ? comidasDelDia : Object.values(comidasDelDia))
    : [];
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-end border-b border-gray-200/40 pb-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Sincronizado exclusivamente con tu PDF oficial.</p>
        </div>
      ...
      {/* Listado de Comidas o Mensaje de Pendiente */}
      {listadoComidas.length > 0 ? (
        ...
        <div className={`rounded-[2rem] p-10 text-center border ${isDarkMode ? 'bg-gray-800 border-gray-700 text-gray-300' : 'bg-white border-gray-100 text-gray-500'}`}>
          <div className="w-12 h-12 bg-amber-50 dark:bg-amber-950/50 text-amber-500 rounded-2xl flex items-center justify-center mx-auto mb-3 text-xl shadow-sm">
            📄
          </div>
          <h3 className="font-bold text-base mb-1">Día pendiente de sincronización</h3>
          <p className="text-xs text-gray-400 max-w-sm mx-auto leading-relaxed">
            El PDF oficial de tu nutriólogo solo tiene registrado el menú del lunes. Los demás días aparecerán aquí conforme los vayas cargando.
          </p>
        </div>
      )}
    </div>
