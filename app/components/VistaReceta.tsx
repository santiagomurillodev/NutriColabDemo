    ...
    return texto.replace(
      ...
      (match, numero, unidad) => {
        ...
        if (nuevoNumero > 1) {
          ...
          else if (unidad.toLowerCase() === 'cucharada') nuevaUnidad = 'cucharadas';
          ...
          <ul className="space-y-3 bg-white border border-gray-100 p-5 rounded-2xl shadow-sm">
            {comida.ingredientes?.map((ing, idx) => (
              <li key={idx} className="flex items-start gap-3 text-gray-600 text-sm">
                <span className="text-emerald-500 font-bold mt-0.5">•</span> {ajustarCantidad(ing)}
              </li>
            ))}
          </ul>
        </div>

        {/* Pasos Consistentes Dinámicos */}
        <div>
          <h3 className="text-sm font-extrabold text-gray-900 uppercase tracking-widest mb-4">
            Preparación
          </h3>
          <div className="space-y-6">
            <div className="flex gap-4">
              ...
              <div>
                <h4 className="font-bold text-gray-900 text-sm">Pasos a seguir</h4>
                <p className="text-gray-500 text-sm mt-1 leading-relaxed whitespace-pre-line">
                  {comida.preparacion || "Sirve y disfruta respetando las cantidades de tu plan."}
                </p>
              </div>
            </div>
          </div>
